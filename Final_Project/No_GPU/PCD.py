...
def compute_class_weights(train_loader, device):
    ...
    label_counts = torch.zeros(7)  # For 7 Pharo categories
    total_samples = 0
...
if __name__ == "__main__":
    ...
    train_df, test_df = load_pharo_data(
        './pharo_train-00000-of-00001.parquet',
        './pharo_test-00000-of-00001.parquet'
    )
    ...
    plt.tight_layout()
    plt.savefig('pharo_confusion_matrices.png')
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('pharo_learning_curves.png')

    # Save detailed results to file
    with open('pharo_model_results.txt', 'w') as f:
    ...
    print("\nResults have been saved to 'pharo_model_results.txt'")
